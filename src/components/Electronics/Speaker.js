...
const Speaker = ({ addToCart, cartItems,  currentPage, setCurrentPage, product}) => {
  ...
  const visibleProducts = speakerData.filter((product) => product.page === currentPage);

const totalPages = Math.max(...speakerData.map((product) => product.page));
              ...
              <Col sm={3} xs={6} key={product.id}>
               <Card className="product-card mb-4 shadow-sm  " >
                  <Card.Body className="d-flex flex-column align-items-center justify-content-center">
                    ...
                    <Link to={`/clickspeaker/${product.id}`}>
