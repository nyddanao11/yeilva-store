...
const CookingItems = ({ cartItems, product, currentPage, setCurrentPage}) => {
  ...
  const visibleProducts = cooking.filter((product) => product.page === currentPage);

const totalPages = Math.max(...cooking.map((product) => product.page));
              ...
              <Col sm={3} xs={6} key={product.id}>
                <Card className="product-card mb-4 shadow-sm  " >
                  <Card.Body className="d-flex flex-column align-items-center justify-content-center">
                   ...
                   <Link to={`/clickcooking/${product.id}`}>
                    <Card.Img 
                    variant="top" 
