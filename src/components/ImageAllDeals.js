...
const ImageAllDeals = ({ url, name, price, id, stock, discount = 0}) => {
  ...
  // FIX: this must be called as a function. Previously referenced as
  // `isProductDiscounted` (the function itself, always truthy), which made
  // the discount badge and strikethrough price render for every product,
  // even ones with discount === 0.
  ...
  if (loading) return <div className="product-skeleton" />;
  if (error) return <div>Error fetching reviews</div>;

  return (
    <Card
      className="product-card border-0 shadow-sm h-100 overflow-hidden transition-all"
      style={{ borderRadius: '15px', transition: 'transform 0.2s ease-in-out' }}
    >
      {/* Badges Overlay */}
      <div
        className="position-absolute top-0 start-0 p-2 d-flex flex-column gap-1"
        style={{ zIndex: 2 }}
      >
        {isProductDiscounted() && (
