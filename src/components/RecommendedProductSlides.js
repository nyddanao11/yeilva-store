...
export default function RecommendedProductSlides ({ addToCart, recommendedProducts, recommendedLoading, recommendedError})  {
  ...
  if (recommendedError) return <div>Error fetching recommendedProducts</div>;
  ...
  const shouldSlide = recommendedProducts.length > maxSlidesNeeded;

  // Dynamically scale down items per view if your inventory is low
  const currentSlidesToShow = Math.min(maxSlidesNeeded, recommendedProducts.length);
