...
const BondPaper = ({ cartItems, product, currentPage, setCurrentPage}) => {
  ...
  const visibleProducts = bondpaper.filter((product) => product.page === currentPage);

const totalPages = Math.max(...bondpaper.map((product) => product.page));
              ...
              <Col sm={3} xs={6} key={product.id} className="d-flex  align-items-center justify-content-center" >
                <Card className="product-card mb-4 shadow-sm  " >
                  <Card.Body className="d-flex flex-column align-items-center justify-content-center">
                   ...
                   <Link to={`/clickbondpaper/${product.id}`}>
