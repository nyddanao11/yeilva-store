...
export default function ClickBestSelling ({ isLoggedIn, bestSellingProducts, youMayLikeProducts })  {
  ...
  useEffect(() => {
    ...
    if ( bestSellingProducts &&  bestSellingProducts.length > 0) {
      const foundProduct = bestSellingProducts.find(p => p.id === parseInt(id));
      if (foundProduct) {
  ...
  }, [id,  bestSellingProducts]); // Depend on 'id' and 'storedProducts'
      ...
      <Container className="my-4 product-page-container">
        ...
        <Row className="my-5">
          <Col xs={12}>
            ...
            {product && <TabbedComponent  productId={product.id} bestSellingProducts={bestSellingProducts}/>}
          </Col>
        </Row>
