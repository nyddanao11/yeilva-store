...
const handleLoginRedirect = () => {
  ...
  const product = allDealsProduct.find(p => p.id === parseInt(id));
    ...
    <>
      <Container className="mt-3">
        <Row className="justify-content-center">
          {product && allDealsProduct && <BreadCrumbDeals productId={product.id} allDealsProduct={allDealsProduct} />}
        ...
        <Row style={{ marginBottom: '60px', marginTop: '60px' }}>
          <Col>
            <TabbedComponent productId={product.id} allDealsProduct={allDealsProduct} />
          </Col>
        </Row>
