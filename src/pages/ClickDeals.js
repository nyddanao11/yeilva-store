...
export default function ClickDeals({ isLoggedIn, allDealsProduct, youMayLikeProducts, mayLikeLoading, mayLikeError }) {
  ...
  // Effect to find the product from  allDealsProduct
  useEffect(() => {
    // Only attempt to find product if  allDealsProduct is available and not empty
    if ( allDealsProduct &&  allDealsProduct.length > 0) {
      const foundProduct = allDealsProduct.find(p => p.id === parseInt(id));
      if (foundProduct) {
  ...
  }, [id,  allDealsProduct]); // Depend on 'id' and 'storedProducts'
    ...
    <>
     <SEO 
      ...
      <Container className="my-4 product-page-container">
        ...
        {product &&  allDealsProduct && (
          <BreadCrumbDeals productId={product.id}  allDealsProduct={ allDealsProduct} />
        )}
     ...
     <Row className="justify-content-center g-4 lg-g-5 product-main-row align-items-start py-4">
        ...
        <Row className="my-5">
          <Col xs={12}>
            ...
            {product && <HighConversionProductDeals  productId={product.id} allDealsProduct={allDealsProduct} handleCheckoutClick={handleCheckoutClick} discountedPriceFormatted={finalPriceFormatted}/>}
          </Col>
        </Row>
