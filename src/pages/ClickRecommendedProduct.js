...
import HighConversionProductRecommended from'../components/ProductTablatureRecommended';
...
export default function ClickRecommendedProduct ({isLoggedIn, recommendedProducts, youMayLikeProducts })  {
  ...
  // Effect to find the product from recommendedProducts
  useEffect(() => {
    // Only attempt to find product if recommendedProducts is available and not empty
    if ( recommendedProducts &&  recommendedProducts.length > 0) {
      const foundProduct = recommendedProducts.find(p => p.id === parseInt(id));
      if (foundProduct) {
  ...
  }, [id,  recommendedProducts]); // Depend on 'id' and 'storedProducts'
      ...
      <Container className="my-4 product-page-container">
       {/* Breadcrumb Navigation */}
        {product &&  recommendedProducts && (
          <BreadCrumbRecommended productId={product.id}  recommendedProducts={ recommendedProducts} />
        )}
      ...
      <div className="d-flex justify-content-center gap-3 align-items-center mt-3 text-muted small">
        ...
        <Row className="my-5">
          <Col xs={12}>
            ...
            {product && <HighConversionProductRecommended productId={product.id} recommendedProducts={recommendedProducts} handleCheckoutClick={handleCheckoutClick} discountedPriceFormatted={discountedPriceFormatted}/>}
          </Col>
        </Row>
