...
import BreadCrumbSearchPage from'../components/BreadCrumbSearchPage';
import HighConversionProductSearch from'../components/ProductTablatureSearchProducts';
...
export default function ClickSearchPage ({ isLoggedIn, youMayLikeProducts })  {
...
if ( searchProducts && searchProducts.length > 0) {
 const foundProduct = searchProducts.find(p => p.id === parseInt(id));
 if (foundProduct) {
...
}, [id, searchProducts]); // Depend on 'id' and 'cartItems'
     ...
     <SEO 
      ...
      <Container className="my-4 product-page-container">
        ...
        {product &&  searchProducts && (
          <BreadCrumbSearchPage productId={product.id}  searchProducts={searchProducts} />
        )}
      ...
      <div className="d-flex justify-content-center gap-3 align-items-center mt-3 text-muted small">
        ...
        <Row className="my-5">
          <Col xs={12}>
            ...
            {product && <HighConversionProductSearch  productId={product.id} searchProducts={searchProducts} handleCheckoutClick={handleCheckoutClick} discountedPriceFormatted={discountedPriceFormatted}/>}
          </Col>
        </Row>
