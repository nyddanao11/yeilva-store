...
export default function FeaturedProduct({ 
  ...
  // Guard against empty array crashes cleanly
  const totalPages = featuredProducts.length > 0 
    ? Math.max(...featuredProducts.map((product) => product.page)) 
    : 1;
  ...
  if (loading) return <div className="text-center py-5 text-muted small fw-bold">Loading digital assets...</div>;
  if (error) return <div className="text-center py-5 text-danger small fw-bold">Error loading premium assets.</div>;
    ...
    <Container className="my-5 px-3 px-md-4">
      ...
      <div className="text-center mb-4.5">
        ...
        <h2 className="fw-black text-dark tracking-tight mb-2" style={{ fontSize: '1.75rem' }}>
          Artificial Intelligence Creative Blueprints
        </h2>
